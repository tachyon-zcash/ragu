...
fn expected(name: &str, point: &str) -> Census {
    let (stage_wires, wires, instance, outputs, demoted, strongly_forced, cheatable) = match name {
        "hashes_1" => (464, 5592, 40, 8, 0, 8, 242),
        "hashes_2" => (464, 8552, 32, 6, 2, 6, 235),
        "inner_collapse" => (1262, 6289, 32, 19, 0, 19, 657),
        "outer_collapse" if point == "seeded" => (464, 2937, 32, 6, 0, 6, 238),
        "outer_collapse" => (464, 2937, 32, 7, 0, 7, 242),
        "compute_v" => (374, 6796, 32, 1, 0, 1, 441),
        "bind_challenges_0" => (374, 7329, 32, 2, 0, 2, 694),
        "bind_challenges_4" if point == "seeded" => (374, 7397, 32, 2, 0, 2, 696),
        "bind_challenges_4" => (374, 7397, 32, 2, 0, 2, 694),
        bind if bind.starts_with("bind_challenges_") => (374, 7350, 32, 2, 0, 2, 694),
        "bind_beta" => (464, 7528, 32, 4, 0, 4, 738),
        "bind_endoscalar" => (376, 2504, 32, 128, 0, 128, 442),
        ...
        "nested_collapse" if point == "seeded" => (1574, 6448, 33, 12, 0, 12, 801),
    ...
    let (pinned, rejected) = match (name, point) {
        ("hashes_1", _) => (102, 140),
        ("hashes_2", _) => (103, 132),
        ("inner_collapse", "seeded") => (231, 426),
        ("inner_collapse", _) => (234, 423),
        ("outer_collapse", "seeded") => (104, 134),
        ("outer_collapse", _) => (102, 140),
        ("compute_v", _) => (14, 427),
        ("bind_challenges_4", "seeded") => (16, 680),
        (bind, _) if bind.starts_with("bind_challenges_") => (14, 680),
        ("bind_beta", _) => (102, 636),
        ("bind_endoscalar", "seeded") => (43, 399),
        ("bind_endoscalar", "leaves") => (53, 389),
        ("bind_endoscalar", "nodes") => (47, 395),
        (step, "seeded") if step.starts_with("native_endoscaling_step_") => (42, 145),
        (step, "leaves") if step.starts_with("native_endoscaling_step_") => (52, 135),
        (step, "nodes") if step.starts_with("native_endoscaling_step_") => (46, 141),
        ("nested_export", "seeded") => (80, 709),
        ("nested_export", "leaves") => (93, 696),
        ("nested_export", "nodes") => (87, 702),
        ("nested_collapse", "seeded") => (81, 720),
        ("nested_collapse", "leaves") => (93, 706),
        ("nested_collapse", "nodes") => (87, 712),
        ("nested_compute_v", "seeded") => (79, 710),
        ("nested_compute_v", "leaves") => (92, 697),
        ("nested_compute_v", "nodes") => (86, 703),
        (_, "seeded") => (42, 162),
        (_, "leaves") => (52, 152),
        (_, "nodes") => (46, 158),
