    ...
    c.bench_function("pcd/poseidon/seed", |b| {
        ...
        b.iter(|| {
            let leaf1 = app
                .seed(
                    &mut rng,
                    nontrivial::WitnessLeafStep {
                        poseidon: Pasta::circuit_poseidon(pasta),
                    },
                    Fp::from(42u64),
                )
                .unwrap();
...
criterion_group! {
    name = poseidon;
    config = Criterion::default().sample_size(50).warm_up_time(std::time::Duration::from_secs(5));
    targets =
