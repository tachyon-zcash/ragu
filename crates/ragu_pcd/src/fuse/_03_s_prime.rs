...
use ragu_circuits::{polynomials::Rank, registry::LagrangeCache, staging::StageExt};
...
impl<C: Cycle, R: Rank, const HEADER_SIZE: usize> Application<'_, C, R, HEADER_SIZE> {
    pub(super) fn compute_s_prime<RNG: CryptoRng>(
        &self,
        rng: &mut RNG,
        lagrange_cache: &LagrangeCache<C::CircuitField>,
        left: &Proof<C, R>,
        right: &Proof<C, R>,
        ...
        let native_registry_wx0_poly = self.native_registry.wx_cached(lagrange_cache, x0);
        ...
        let native_registry_wx1_poly = self.native_registry.wx_cached(lagrange_cache, x1);
