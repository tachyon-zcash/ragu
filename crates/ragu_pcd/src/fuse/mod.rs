...
impl<C: Cycle, R: Rank, const HEADER_SIZE: usize> Application<'_, C, R, HEADER_SIZE> {
    ...
    pub fn fuse<'source, RNG: CryptoRng, S: Step<C>>(
        ...
        let lagrange_cache = self.native_registry.cache_lagrange(*w.value().take());

        let s_prime = self.compute_s_prime(rng, &lagrange_cache, &left, &right)?;
        ...
        let (error_m, error_m_witness, claims) =
            self.compute_errors_m(rng, &lagrange_cache, &y, &z, &left, &right)?;
