...
impl<F: PrimeField> ChildEvaluationsWitness<F> {
    ...
    pub fn from_proof<C: Cycle<CircuitField = F>, R: Rank>(
        ...
        ChildEvaluationsWitness {
            preamble: proof.preamble.native_rx.poly().eval(xz),
            error_m: proof.error_m.native_rx.poly().eval(xz),
            error_n: proof.error_n.native_rx.poly().eval(xz),
            query: proof.query.native_rx.poly().eval(xz),
            eval: proof.eval.native_rx.poly().eval(xz),
            application: proof.application.rx.poly().eval(xz),
            hashes_1: proof.circuits.hashes_1.poly().eval(xz),
            hashes_2: proof.circuits.hashes_2.poly().eval(xz),
            partial_collapse: proof.circuits.partial_collapse.poly().eval(xz),
            full_collapse: proof.circuits.full_collapse.poly().eval(xz),
            compute_v: proof.circuits.compute_v.poly().eval(xz),
            a_poly_at_xz: proof.ab.a.poly().eval(xz),
            b_poly_at_x: proof.ab.b.poly().eval(x),
            child_registry_xy_at_current_w: proof.query.registry_xy.poly().eval(w),
