...
impl<'dr, D: Driver<'dr>, C: Cycle<CircuitField = D::F>> Output<'dr, D, C> {
    ...
    pub fn alloc_from_proof<R: Rank>(
        ...
        let nested_preamble_commitment = Point::alloc(
            dr,
            proof.as_ref().map(|p| p.preamble.nested_rx.commitment()),
        )?;
        ...
        let nested_s_prime_commitment = Point::alloc(
            ...
            proof
                .as_ref()
                .map(|p| p.s_prime.nested_s_prime_rx.commitment()),
        )?;
        ...
        let nested_error_m_commitment =
            Point::alloc(dr, proof.as_ref().map(|p| p.error_m.nested_rx.commitment()))?;
        ...
        let nested_error_n_commitment =
            Point::alloc(dr, proof.as_ref().map(|p| p.error_n.nested_rx.commitment()))?;
