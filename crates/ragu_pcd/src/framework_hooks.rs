...
//!   `claimed_y`. Each call *induces a stage*: the gadget's wires become a
...
//!   of the book). The succinct commitment is hashed to derive `challenge_x`,
//!   and `claimed_y` is the matching evaluation claim. The simple model is one
//!   stage per call; batching consecutive calls into a single stage is a future
//!   optimization. See [`InducedStage`].
//!
...
/// commitment is hashed to derive `challenge_x`, and `claimed_y` is the
/// corresponding evaluation claim.
///
...
pub struct InducedStage {
    ...
    // `point`, `challenge_x`, and `claimed_y` — that fuse resolves once the
    // stage is committed. Adding these will parameterize this struct by
    // `<'dr, D, C>`, matching [`FrameworkHooks`].
}
...
impl<'dr, D: Driver<'dr>, C: CurveAffine<Base = D::F>> FrameworkHooks<'dr, D, C> {
    ...
    /// independently, hashing the commitment to obtain `challenge_x`. The
    ...
    pub fn derive_challenge<G: Gadget<'dr, D>>(
        ...
        // TODO: allocate the three outputs — the nested-curve commitment
        // `point`, `challenge_x`, and `claimed_y` — against deferred witnesses,
        // store their handles on the recorded `InducedStage`, and return them.
        // This needs the staging transform plus the deferred-value mechanism
        // (the in-circuit hash of the stage commitment, à la
        // `internal/native/circuits/hashes_1.rs`), which is future framework
        // work.
        todo!("allocate and return deferred (point, challenge_x, claimed_y)")
    }
