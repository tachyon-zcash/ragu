...
pub(crate) struct SlotStage<C: CurveAffine, const NUM_SLOTS: usize>(core::marker::PhantomData<C>);
    ...
    for SlotStage<C, NUM_SLOTS>
{
        ...
        Self: 'dr,
    {
        ...
        Ok(FixedVec::new(v).expect("length should be correct"))
    }
}
...
#[derive(Clone)]
pub(crate) struct Endoscaling<C: CurveAffine, R: Rank, const NUM_SLOTS: usize> {
    pub a: Read,
    pub b: Read,
    pub c: Read,
    pub d: Read,
    ...
    pub(crate) _marker: core::marker::PhantomData<(C, R)>,
}

pub(crate) struct EndoscalingWitness<C: CurveAffine, const NUM_SLOTS: usize> {
    pub(crate) endoscalar: Uendo,
    pub(crate) slots: [C; NUM_SLOTS],
    pub(crate) input: C,
}

pub(crate) struct EndoscalingInstance<C: CurveAffine, const NUM_SLOTS: usize> {
    pub(crate) input: C,
    pub(crate) output: C,
}

#[derive(Gadget, Write)]
pub(crate) struct EndoscalingOutput<'dr, D: Driver<'dr>, C: CurveAffine> {
    #[ragu(gadget)]
    input: Point<'dr, D, C>,
    #[ragu(gadget)]
    output: Point<'dr, D, C>,
...
mod tests {
    ...
    fn test_endoscaling_circuit() -> Result<()> {
        ...
        let (final_rx, output) = staged_circuit.rx::<R>(EndoscalingWitness {
            endoscalar,
            slots: values,
            input,
        })?;
