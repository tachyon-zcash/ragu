...
impl<NestedCurve> Default for EChallengeDerivationStagedCircuit<NestedCurve> {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    for EChallengeDerivationStagedCircuit<NestedCurve>
{
    ...
    fn witness<'a, 'dr, 'source: 'dr, D: Driver<'dr, F = NestedCurve::Base>>(
        ...
        let _txz = dr.routine(evaluate_txz, (x_challenge.clone(), z_challenge))?;
...
mod tests {
    ...
    fn test_invalid_mu_challenge_fails() {
        ...
        Simulator::simulate(witness, |dr, witness| {
            let circuit = EChallengeDerivationStagedCircuit::<EpAffine>::new();
            let stage_builder =
                StageBuilder::<'_, '_, _, Rank, (), EStage<EpAffine, NUM_EVALS>>::new(dr);
