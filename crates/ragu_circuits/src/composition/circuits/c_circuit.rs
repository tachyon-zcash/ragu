...
use crate::polynomials::{CrossProductsLen, KyPolyLen};
use crate::{
    composition::staging::{
        g_stage::KYStage,
...
impl<NestedCurve, const HEADER_SIZE: usize, const NUM_CIRCUITS: usize> Default
    for DCValueComputationStagedCircuit<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>
{
    fn default() -> Self {
        Self(PhantomData)
    }
...
impl<NestedCurve, const HEADER_SIZE: usize, const NUM_CIRCUITS: usize>
    DCValueComputationStagedCircuit<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>
{
    pub fn new() -> Self {
        Self::default()
    }
...
> StagedCircuit<NestedCurve::Base, R>
    for DCValueComputationStagedCircuit<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>
{
    type Final = KYStage<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>;
    type Instance<'src> = UnifiedRecursionInstance<NestedCurve>;
    type Witness<'w> = DCValueComputationWitness<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>;
    ...
    fn witness<'a, 'dr, 'source: 'dr, D: Driver<'dr, F = NestedCurve::Base>>(
        ...
        let (stage_output, dr) = dr.add_stage::<KYStage<NestedCurve, HEADER_SIZE, NUM_CIRCUITS>>(
            witness.view().map(|w| w.ky_coeffs.clone()),
        )?;
...
mod tests {
    ...
    fn test_c_value_computation_matches_reference() -> Result<()> {
        ...
        let ky_degree = KyPolyLen::<TEST_HEADER_SIZE>::len(); // 4 coefficients per circuit
        let total_ky_coeffs = TotalKyCoeffsLen::<TEST_HEADER_SIZE, TEST_NUM_CIRCUITS>::len(); // 12
        let num_cross_products = CrossProductsLen::<TEST_NUM_CIRCUITS>::len(); // 6
        ...
        let witness: DCValueComputationWitness<EpAffine, TEST_HEADER_SIZE, TEST_NUM_CIRCUITS> =
        ...
        let circuit =
            DCValueComputationStagedCircuit::<EpAffine, TEST_HEADER_SIZE, TEST_NUM_CIRCUITS>::new();
