...
impl<F: PrimeField> Domain<F> {
    ...
    /// $\ell_i(x)$ for the domain generated by powers of $\omega$
    /// corresponding to the indices determined by the deterministic bit-reversal scheme
    /// described in the book as "rolling domain extensions", and implemented in [`ragu_circuits::mesh`]
    pub fn ell(&self, x: F, amount: usize) -> Option<Vec<F>> {
        ...
        // Compute the base: ROOT_OF_UNITY^(2^shift)
        let base = F::ROOT_OF_UNITY.pow([1u64 << shift]);
        // Use a running accumulator to compute successive powers: 1, base, base^2, base^3, ...
        let omega_powers: Vec<F> = core::iter::once(F::ONE)
            .chain((1..amount).scan(base, |acc, _| {
                let result = *acc;
                *acc *= base;
                Some(result)
            }))
